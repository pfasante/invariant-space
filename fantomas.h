/* The following code is adapted from the reference implementation by LS-Design authors. The reference implementation comes with the following header.
	* LS Block Cipher - Implementation
	* This code is placed in the public domain
	* For more information, feedback or questions, please refer to our website:
	* https://sites.google.com/site/LSBlockCipher
*/
...
void fantomas_const(void *data, int i) {
  uint16_t s[8] = {};
  s[0] ^= LBox1[i+1];
  memcpy(data,s,16);
}

static void fantomas(void *data) {
  uint16_t *s = (uint16_t*) data;
  int i;
  SBOX(s);
  for (i=0; i<8; i++)
    s[i] = LBox2[s[i]>>8] ^ LBox1[s[i]&0xff];
}

void fantomas_print_state(void *data) {
  print_state((uint16_t*) data);
}
...
void Encrypt(uint8_t input[16], uint8_t key[16]) {
...
void Decrypt(uint8_t input[16], uint8_t key[16]) {
  // Access state as 16-bit values
  // Assumes little-endian machine
  uint16_t *data = (uint16_t*) input;
  uint16_t *k    = (uint16_t*) key;
